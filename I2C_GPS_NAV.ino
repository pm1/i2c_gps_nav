...
void setup() {
  ...
  Serial.begin(GPS_SERIAL_SPEED); // Start with GPS native speed
...
Serial.write("$PMTK220,100*2F\r\n");
...
#if defined(UBLOX)  
#if defined(NEO6_NOT_CONFIGURED_AT_POWERON)
  ...
  //set WAAS to EGNOS
  PROGMEM prog_uchar conf14[]={0xB5,0x62,0x06,0x16,0x08,0x00,0x03,0x07,0x03,0x00,0x51,0x08,0x00,0x00,0x8A,0x41};
  ...
  i2c_dataset.nav_crosstrack_gain = CROSSTRACK_GAIN; // Crosstrack gain = 1
  i2c_dataset.nav_speed_min       = NAV_SPEED_MIN; // cm/s
  i2c_dataset.nav_speed_max       = NAV_SPEED_MAX; // cm/s
  i2c_dataset.nav_bank_max        = NAV_BANK_MAX; // 30deg
  i2c_dataset.wp_radius           = WP_RADIUS;   //cm -> 2m
